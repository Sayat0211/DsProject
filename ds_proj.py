    ...
    if not is_admin:
        ...
        if st.button("✨ Find Best Match", key="desc_button"):
            if query.strip() == "":
                ...
                # Рекомендация машин на основе выбранных характеристик
                st.markdown("---")
                st.markdown("### 🚗 Рекомендуемые автомобили:")
                
                try:
                    # Фильтрация данных по выбранным характеристикам
                    filtered_cars = raw_data.copy()
                    
                    # Применяем фильтры напрямую (датасет на русском)
                    ...
                    if not filtered_cars.empty:
                        ...
                        # Добавляем кнопку для просмотра всех вариантов
